...
```{r}
# glimpse the mingo data
glimpse(mingo)
```

```{r}
# summary of mingo data
summary(mingo)
...
```{r}
mingo_working <- mingo %>% mutate(distributor_address_complete = paste0(reporter_address1, ", ", reporter_address2 , reporter_city, ", ", reporter_state, " ", reporter_zip))
```
...
# made a new column to indicate if a shipment is in state or out of state
mingo_working <- mingo %>% mutate(in_state_distribution = 
 case_when(reporter_state > "WV" ~ "yes")) 
...
# the most significant combo is Actavis pharma, Miami-Luken, and Tug Valley Pharmacy. 
...
# figure out if the three similarly-named companies are actually the same company

mingo %>%
group_by(reporter_address1) %>%
filter(reporter_name == "Cardinal$")
  
# i have no idea how to do this and i've spent like 45 minutes trying and i think this might be it? 
...
```{r}
# in our first R lesson we learned about str_detect() function. I tried to apply it in the last question but I can't seem to make it work. I think this would be a way to do it though- we should be able to "Cardinal$" as the dollar sign would then include all the cardinals together. I think. 
```
