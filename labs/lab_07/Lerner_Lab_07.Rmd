...
```{r}
# Comment
# It is possible some of these places show up in the pills table, but not in the population table, because a lot of these areas may not be habitable. Places in Puerto Rico and the Virgin Islands have been decimated by extreme weather in the past, and it's possible that at some point during the 2006-2012 database tracking, there was no population info available or it was just impossible to track. For the most part, it appears that the records that don't show up are the ones from U.S. colonies, not states. The states that do show up, such as Montgomery County AR, could be because the data may be merged across different Montgomery Counties in different states?
```
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per person', title="Rust Belt States receive highest rate of opioids per person", subtitle = "2009 population, U.S. census", caption = "Source: U.S. Census ACS, Arcos DEA database") + 
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Rate of opioids per person rises in all states between 2006-2012", subtitle = "Rust Belt and West Coast states saw most significant rise", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
# Map Code Here
pills_population_per_county <- pills_population %>%
...
geo_pills_population_county_2009 <- county_geodata_shifted %>%
  inner_join(pills_population_per_county, by=c("GEOID" = "countyfips"))
...
pills_population_per_county_MD <- pills_population %>%
...
pills_population_per_county_MD <- county_geodata_shifted %>%
 inner_join(pills_population_per_county_MD, by=c("GEOID" = "countyfips"))


pills_population_per_county_MD %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
