...
```{r}
# okay now i do want to do the same data but nationwide, not just focused on vermont 
...
# set up buyer data 
buyer_annual_by_year <- buyer_annual_by_year %>%
  rename(state=buyer_state) %>%
  group_by(year, state) %>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit))

#this is the rate of pills per person per year per state
pills_per_person_national <- state_population_per_year_UPDATE %>%
  full_join(buyer_annual_by_year, by="state") %>%
  mutate(rate=total_pills/population_average) #%>%
 #replace(state, c(AK) c("Alaska"))

# join pills_per_person_national with state names 
pills_per_person_national_x <- pills_per_person_national %>%
  full_join(state_names, by=c("state"="abb")) %>%
  select(state=title, population_average, year, total_pills, rate) 

#set up children in foster care 
children_in_foster_care_STATES <- children_in_foster_care_STATES %>%
  filter(year>="2006",
         year<="2012")

# everything in pills_per_person_national looks fine, but i cant make the datasets merge well because I can't get AK to equal alaska, etc. How can I fix this?

# now do the rate of foster children per person per year per state
foster_rate_pop_national <- children_in_foster_care_STATES %>%
  full_join(state_population, by="state") %>%
  #filter(state=="Vermont") %>%
  mutate(rate=(total_kids/population_average)*1000) %>%
  filter(year>="2006") %>%
...
#next i need to do an inner join of these two data plots, and then do a scatterplot for them. 

scatter_pills_kids_national <- foster_rate_pop_national %>%
  inner_join(pills_per_person_national_x, by=c("state","year"))  %>%
 # select(state.y, year, population_average.x, total_pills, total_kids, rate.x, rate.y) %>%
  rename(pills_per_person=rate.y) %>%
  rename(foster_per_person=rate.x) 

#so that's the end dataset. Now let's do a scatterplot!
# this dataset is wrong and acomplete and total mess so ¯\_(ツ)_/¯

ggplot(scatter_pills_kids_national) +
  geom_point(aes(foster_per_person, pills_per_person)) +
  geom_smooth(aes(foster_per_person, pills_per_person), method = "lm", se = FALSE)  +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma)  +
  ...
  geom_text_repel(aes(foster_per_person, pills_per_person, label=foster_per_person),
                  subset(scatter_pills_kids)) +
  labs(x="rate of fosters per person", y="pills per person", title="XXX", caption = "Source: DEA ARCOS database, via Washington Post")
