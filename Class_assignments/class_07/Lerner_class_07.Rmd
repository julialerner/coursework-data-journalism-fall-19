...
```{r}
# create a graph that shows year-over-year pill totals in Logan, WV, 2006-2012. 
# create the data the bar chart will pull from 

# logan_pills_per_year <- arcos_county_pills_per_year %>%
  #select(year, dosage_unit, buyer_state, buyer_county) %>%
...
```{r}
# this is the data the chart will pull from 
...
```{r}
# It looks like in some counties, the number of opioids shipped continue to rise, while in others, the rates shipped leveled off or dropped. Kanawha, Wood, and Raleigh started off receiving fewer pills in 2006 than they did in 2012, while Cabell and Logan, and (sort of) Wood were fairly level throughout 2006-2012. Mingo is the only county that saw a significant drop in opioids sent over the years. 
```
...
```{r}
# Get pills for Ohio for 2009
# OH_county_pills_2009 <- arcos_county_pills_per_year %>%
#  filter(year=="2009", buyer_state=="OH")
```

```{r}
# Get population for OH for 2009
#OH_county_population_2009 <- county_population(key = key) %>%
#  clean_names() %>%
#  filter(year=="2009", buyer_state=="OH")
```

```{r}
# Join together pills and population for OH for 2009
#OH_county_pills_population_2009 <- OH_county_pills_2009 %>%
#  inner_join(OH_county_population_2009)
```

```{r}
# Create the pills per person figure for ohio for 2009
# OH_county_pills_population_2009 <- OH_county_pills_population_2009 %>%
 # mutate(pills_per_person = dosage_unit/population) %>%
#  select(buyer_county, buyer_state, countyfips, pills_per_person) %>%
...
```{r}
#OH_county_pills_population_income_2009 <- #OH_county_pills_population_2009 %>%
#  inner_join(county_median_household_income, by = c("countyfips"="GEOID"))
```

```{r}
# Build the Graphic. 
#ggplot(OH_county_pills_population_income_2009) +
#  geom_point(aes(estimate, pills_per_person)) +
#  geom_smooth(aes(estimate, pills_per_person), method = "lm", se = FALSE)  +
#  scale_y_continuous(labels = comma) +
#  scale_x_continuous(labels = comma)  +
#  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
#  geom_text_repel(aes(estimate, pills_per_person, label=buyer_county),
 #                 subset(OH_county_pills_population_income_2009)) +
 # labs(x="2009 median household income", y="2009 pills per person", title="Less well off counties tend have higher opioid rate", caption = "Source: DEA ARCOS database, via Washington Post")
```  
